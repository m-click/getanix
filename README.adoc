...
effect that `nix-build` by default creates both, the `env` and the
tarball, which is certainly not what we want.
...
[NOTE]
We need the `ls` option `-L` to see the actual archive files rather
than the symlinks.
